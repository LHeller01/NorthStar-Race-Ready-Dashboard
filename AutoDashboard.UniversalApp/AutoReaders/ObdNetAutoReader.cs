...
namespace AutoDashboard.UniversalApp.AutoReaders
{
    class ObdNetAutoReader : IAutoReader, IDisposable
    {
        ...
        public async Task Update()
        {
            ...
            var fuellevel = await _dev.RequestDataAsync<FuelTankLevelInput>();
            if (fuellevel != null)
            {
                Debug.WriteLine(fuellevel.ToString());
            }
        }
        ...
        public Task<T> Get<T>() where T : IAutoReading
        {
            ...
            if (typeof(T) == typeof(FuelLevel))
            {
                return Task.FromResult(new FuelLevel(_fuellevelvalue)) as Task<T>;
            }
